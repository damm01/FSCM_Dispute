*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVFIFSCM_CAT_REA
*   generation date: 28.08.2019 at 16:21:18
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVFIFSCM_CAT_REA    .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVFIFSCM_CAT_REA'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVFIFSCM_CAT_REA_EXTRACT
TOTAL = ZVFIFSCM_CAT_REA_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVFIFSCM_CAT_REA'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVFIFSCM_CAT_REA-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVFIFSCM_CAT_REA_EXTRACT
TOTAL = ZVFIFSCM_CAT_REA_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVFIFSCM_CAT_REA-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVFIFSCM_CAT_REA'
...
TOTAL = ZVFIFSCM_CAT_REA_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVFIFSCM_CAT_REA-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVFIFSCM_CAT_REA-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVFIFSCM_CAT_REA'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVFIFSCM_CAT_REA-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVFIFSCM_CAT_REA_EXTRACT
TOTAL = ZVFIFSCM_CAT_REA_TOTAL
