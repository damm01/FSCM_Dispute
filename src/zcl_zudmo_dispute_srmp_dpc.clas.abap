class ZCL_ZUDMO_DISPUTE_SRMP_DPC definition
  public
...
protected section.
  ...
  methods ZC_DISPUTE_SRMPR_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZUDMO_DISPUTE_SRMP_MPC=>TS_ZC_DISPUTE_SRMPROTOCOLTYPE
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZC_DISPUTE_SRMPR_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZC_DISPUTE_SRMPR_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
  ...
  methods ZC_DISPUTE_SRMPR_GET_ENTITYSET
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZC_DISPUTE_SRMPR_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZUDMO_DISPUTE_SRMP_DPC IMPLEMENTATION.
...
*&* This class has been generated on 24.10.2019 12:23:22 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZUDMO_DISPUTE_SRMP_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA zc_dispute_srmpr_create_entity TYPE zcl_zudmo_dispute_srmp_mpc=>ts_zc_dispute_srmprotocoltype.
...
*-------------------------------------------------------------------------*
*             EntitySet -  zc_dispute_srmprotocol
*-------------------------------------------------------------------------*
     WHEN 'zc_dispute_srmprotocol'.
*     Call the entity set generated method
    zc_dispute_srmpr_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = zc_dispute_srmpr_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = zc_dispute_srmpr_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*-------------------------------------------------------------------------*
      when 'zc_dispute_srmprotocol'.
*     Call the entity set generated method
     zc_dispute_srmpr_delete_entity(
          EXPORTING iv_entity_name     = iv_entity_name
                    iv_entity_set_name = iv_entity_set_name
                    iv_source_name     = iv_source_name
                    it_key_tab         = it_key_tab
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zc_dispute_srmpr_get_entity TYPE zcl_zudmo_dispute_srmp_mpc=>ts_zc_dispute_srmprotocoltype.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          zc_dispute_srmpr_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zc_dispute_srmpr_get_entity
                         es_response_context = es_response_context
          ).

        IF zc_dispute_srmpr_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zc_dispute_srmpr_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZUDMO_DISPUTE_SRMP_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA zc_dispute_srmpr_get_entityset TYPE zcl_zudmo_dispute_srmp_mpc=>tt_zc_dispute_srmprotocoltype.
 DATA lv_entityset_name TYPE string.
...
*-------------------------------------------------------------------------*
   WHEN 'zc_dispute_srmprotocol'.
*     Call the entity set generated method
      zc_dispute_srmpr_get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = zc_dispute_srmpr_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = zc_dispute_srmpr_get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zc_dispute_srmpr_update_entity TYPE zcl_zudmo_dispute_srmp_mpc=>ts_zc_dispute_srmprotocoltype.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*-------------------------------------------------------------------------*
      WHEN 'zc_dispute_srmprotocol'.
*     Call the entity set generated method
          zc_dispute_srmpr_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zc_dispute_srmpr_update_entity
          ).
       IF zc_dispute_srmpr_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zc_dispute_srmpr_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_zc_dispute_srmprotocol_1 TYPE zc_dispute_srmprotocol ##NEEDED. " reference for where-used list

    DATA(lv_sadl_xml) =
               |<?xml version="1.0" encoding="utf-16"?>| &
               |<sadl:definition xmlns:sadl="http://sap.com/sap.nw.f.sadl" syntaxVersion="" >| &
               | <sadl:dataSource type="CDS" name="ZC_DISPUTE_SRMPROTOCOL" binding="ZC_DISPUTE_SRMPROTOCOL" />| &
               |<sadl:resultSet>| &
               |<sadl:structure name="zc_dispute_srmprotocol" dataSource="ZC_DISPUTE_SRMPROTOCOL" maxEditMode="RO" exposure="TRUE" >| &
    ...
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20191024102321
               iv_uuid              = 'ZUDMO_DISPUTE_SRMPROTOCOL'
               io_context           = me->mo_context ).
  endmethod.


  method ZC_DISPUTE_SRMPR_CREATE_ENTITY.
  ...
  method ZC_DISPUTE_SRMPR_DELETE_ENTITY.
    if_sadl_gw_dpc_util~get_dpc( )->delete_entity( io_tech_request_context ).
  endmethod.


  method ZC_DISPUTE_SRMPR_GET_ENTITY.
  ...
  method ZC_DISPUTE_SRMPR_UPDATE_ENTITY.
